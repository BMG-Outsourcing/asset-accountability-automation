...
def get_logo_b64() -> str:
    logo_candidates = [
        Path(r"C:\Users\Ac\asset-accountability-system\images\logo.png"),
        Path("images/logo.png"),
        Path("src/images/logo.png"),
        Path("/mnt/user-data/uploads/UL4W99QRRJ5XU8VFJU9R-58801410-removebg-preview.png"),
    ]
...
TEMPLATE_PATH     = r"src\Equipment Accountability Form (Work From Home).dotx"
TEMPLATE_FALLBACK = "/mnt/user-data/uploads/Equipment_Accountability_Form__Work_From_Home_.dotx"
...
def get_template_path() -> Path | None:
    for p in [TEMPLATE_PATH, TEMPLATE_FALLBACK]:
        if Path(p).exists():
            return Path(p)
    return None
...
def main():
    ...
    tpl = get_template_path()
    if tpl is None:
        st.error("Word template (.dotx) not found. Make sure it is in the correct folder.")
        step_close()
        return
